...
/* -------TESTS------- */
void test_merge_sort_on_record_array(int field) {
  int size;
  Record** array = read_data(&size,ARRAY_LENGTH);

  start_timer(" Merge sort");
  if(field == 1){
    merge_sort((void**) array, size, compare_field1_ptr);
  }else if(field == 2){
    merge_sort((void**) array, size, compare_field2_ptr);
  }else if(field == 3){
    merge_sort((void**) array, size, compare_field3_ptr);
  }
  end_timer();
...
int main(int argc, char const *argv[]) {
  ...
  test_merge_sort_on_record_array(field);
  printf("\n<<-- Sorted by field%d -->>\n\n",field);
  return 0;
}
